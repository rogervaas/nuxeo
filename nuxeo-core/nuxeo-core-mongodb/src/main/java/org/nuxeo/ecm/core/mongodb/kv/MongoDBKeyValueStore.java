...
public class MongoDBKeyValueStore implements KeyValueStoreProvider {
    ...
    public byte[] get(String key) {
        Document doc = coll.find(Filters.eq(ID_KEY, key)).first();
        if (doc == null) {
            if (log.isTraceEnabled()) {
    ...
    public void put(String key, byte[] bytes, long ttl) {
        if (bytes == null) {
            ...
            coll.deleteOne(new Document(ID_KEY, key));
        } else {
            Document filter = new Document(ID_KEY, key);
    ...
    protected void setTTL(Document doc, long ttl) {
        if (ttl != 0) {
            doc.append(TTL_KEY, new Date(System.currentTimeMillis() + ttl * 1000));
        }
    }
    ...
    public boolean setTTL(String key, long ttl) {
        ...
        Document update;
        if (ttl == 0) {
            update = new Document(MONGODB_UNSET, new Document(TTL_KEY, ONE));
        } else {
            Document doc = new Document();
            setTTL(doc, ttl);
            update = new Document(MONGODB_SET, doc);
        }
        if (log.isTraceEnabled()) {
    ...
    public boolean compareAndSet(String key, byte[] expected, byte[] value) {
        if (expected == null && value == null) {
            ...
            Document filter = new Document(ID_KEY, key).append(VALUE_KEY, toStorage(expected));
